...
void load_root(u32 hd_dev_id, void *fs_root)
{
	...
	else if (hd_dev_id >= HD_DEV_ID_HDA_MIN && hd_dev_id <= HD_DEV_ID_HDA_MAX)
	{
		...
		{
			if (sys_var->pts[i].device == hd_dev_id)
			{
				u32 lba = sys_var->pts[i].start;
...
u32 hda_alloc_sec(u32 dev_id)
{
	...
	for (int i = 0; i < sys_var->pt_count; i++)
	{
		if (sys_var->pts[i].device == dev_id)
		{
			...
			{
				hd_rw(sys_var->pts[i].bitmap_start + j, HD_READ, bitmap);
				for (int k = 0; k < 0x200; k++)
				{
					for (int l = 0; l < 8; l++)
					{
						...
						{
							bitmap[k] |= (0x1 << l);
							hd_rw(sys_var->pts[i].bitmap_start + j, HD_WRITE, bitmap);
...
void hda_free_sec(u32 hd_dev_id, u32 lba)
{
	...
	for (int i = 0; i < sys_var->pt_count; i++)
	{
		if (sys_var->pts[i].device == hd_dev_id)
		{
			u32 logic_lba = lba - sys_var->pts[i].start;
			u32 ind = logic_lba / 8 / 0x200;
			hd_rw(sys_var->pts[i].bitmap_start + ind, HD_READ, bitmap);
			bitmap[logic_lba / 8] |= (0x1 << logic_lba % 8);
			hd_rw(sys_var->pts[i].bitmap_start + ind, HD_WRITE, bitmap);
			break;
		}
	}
}
