...
{
	int status = fs_find_path(path_name, ret_dev_id, fs);
	...
	if ((*fs)->owner != uid && (*fs)->group != gid)
	{
		if ((((*fs)->mode >> 2) & 0x1) == 0)
		{
			status = FS_STATUS_NO_PERMISSION;
		}
	}
	else if ((*fs)->owner != uid && (*fs)->group == gid)
	{
		if ((((*fs)->mode >> 5) & 0x1) == 0)
		{
			status = FS_STATUS_NO_PERMISSION;
...
//			{
//				//创建此文件夹
//				child_no = fs_create_fs(dev_id, parent_no, folder_name, FS_FOLDER_MODE);
//			}
//			//找到子文件块，将其设定为下一级父文件块编号
//			parent_no = child_no;
//		}
...
u32 fs_find_sub_fs(u32 dev_id, u32 parent_no, char *folder_name)
{
	...
	read_block(dev_id, fs->address, fst_addrs);
	//循环一级地址列表
				...
				if (sec_addrs->addr_no[j] != 0)
				{
					...
					for (int k = 0; k < FS_ADDR_COUNT; ++k)
					{
						if (thrid_addrs->addr_no[k] != 0)
						{
							...
							read_block(dev_id, thrid_addrs->addr_no[k], sub_folder);
...
void fs_del_fs_no(u32 dev_id, u32 p_no, u32 del_no)
{
	//处理fs_parent
	s_fs *fs_parent = alloc_mm(sizeof(s_fs));
	read_block(dev_id, p_no, fs_parent);

	s_fs *p_fst_addrs = alloc_mm(sizeof(s_fs));
	read_block(dev_id, fs_parent->address, p_fst_addrs);

	for (int i = 0; i < FS_ADDR_COUNT; ++i)
	{
		s_fs *p_sec_addrs = alloc_mm(sizeof(s_fs));
		if (p_fst_addrs->addr_no[i] != 0)
		{
			read_block(dev_id, p_fst_addrs->addr_no[i], p_sec_addrs);
			for (int j = 0; j < FS_ADDR_COUNT; ++j)
			{
				s_fs *p_thrid_addrs = alloc_mm(sizeof(s_fs));
				if (p_sec_addrs->addr_no[j] != 0)
				{
					//三级地址列表
					read_block(dev_id, p_sec_addrs->addr_no[j], p_thrid_addrs);
					for (int k = 0; k < FS_ADDR_COUNT; ++k)
					{
						if (p_thrid_addrs->addr_no[k] == del_no)
						{
							p_thrid_addrs->addr_no[k] = 0;
							break;
						}
					}
					write_block(dev_id, p_sec_addrs->addr_no[j], p_thrid_addrs);
				}
				free_mm(p_thrid_addrs, sizeof(s_fs));
			}
		}
		free_mm(p_sec_addrs, sizeof(s_fs));
	}
	free_mm(p_fst_addrs, sizeof(s_fs));
	free_mm(fs_parent, sizeof(s_fs));
}
			...
			for (int j = 0; j < FS_ADDR_COUNT; ++j)
			{
				...
				if (sec_addrs->addr_no[j] != 0)
				{
					...
					for (int k = 0; k < FS_ADDR_COUNT; ++k)
					{
						if (thrid_addrs->addr_no[k] != 0)
						{
							free_sec(dev_id, thrid_addrs->addr_no[k]);
						}
					}
					free_sec(dev_id, sec_addrs->addr_no[j]);
				}
				free_mm(thrid_addrs, sizeof(s_fs));
			}
			free_sec(dev_id, fst_addrs->addr_no[i]);
		}
		free_mm(sec_addrs, sizeof(s_fs));
	}
	free_mm(fst_addrs, sizeof(s_fs));
...
//			{
//				//创建文件
//				child_no = fs_create_fs(dev_id, parent_no, fs_name, FS_FOLDER_MODE);
//			}
//			//找到子文件块，将其设定为下一级父文件块编号
//			parent_no = child_no;
//		}
...
//			fs_name[j++] = '\0';
//			//创建文件
//			fs_create_fs(dev_id, parent_no, fs_name, FS_FILE_MODE);
//		}
//	}
...
//{
//	return fs_create_file(file_name);
//}
...
int f_write_block(s_file *fp)
{
	...
	{
		//创建一级磁盘指针
		fp->fs.address = alloc_sec(fp->dev_id);
		fs_empty_data(fst_addrs);
		//写入一级指针块数据
		write_block(fp->dev_id, fp->fs.address, fst_addrs);
		//写入文件根块数据
		write_block(fp->dev_id, fp->fs.dot, &fp->fs);
	}
	read_block(fp->dev_id, fp->fs.address, fst_addrs);
	...
	{
		//创建二级磁盘指针
		fst_addrs->addr_no[fp->first_index] = alloc_sec(fp->dev_id);
		fs_empty_data(sec_addrs);
		//写入二级指针块数据
		write_block(fp->dev_id, fst_addrs->addr_no[fp->first_index], sec_addrs);
		//写入一级指针块数据
		write_block(fp->dev_id, fp->fs.address, fst_addrs);
	}
	read_block(fp->dev_id, fst_addrs->addr_no[fp->first_index], sec_addrs);
	...
	{
		//创建三级磁盘指针
		sec_addrs->addr_no[fp->sec_index] = alloc_sec(fp->dev_id);
		fs_empty_data(thrid_addrs);
		//写入三级指针块数据
		write_block(fp->dev_id, sec_addrs->addr_no[fp->sec_index], thrid_addrs);
		//写入二级指针块数据
		write_block(fp->dev_id, fst_addrs->addr_no[fp->first_index], sec_addrs);
	}
	read_block(fp->dev_id, sec_addrs->addr_no[fp->sec_index], thrid_addrs);

	if (thrid_addrs->addr_no[fp->third_index] == 0)
	{
		//创建磁盘文件数据块
		thrid_addrs->addr_no[fp->third_index] = alloc_sec(fp->dev_id);
		//写入三级指针块数据
		write_block(fp->dev_id, sec_addrs->addr_no[fp->sec_index], thrid_addrs);
	}
	//写入文件块数据
	write_block(fp->dev_id, thrid_addrs->addr_no[fp->third_index], fp->buff);
...
int f_read_block(s_file *fp, int block_ind)
{
	...
	//读入文件块数据
	read_block(fp->dev_id, thrid_addrs->addr_no[fp->third_index], fp->buff + (FS_DATA_COUNT * block_ind));
				...
				if (sec_addrs->addr_no[j] != 0)
				{
					...
					for (int k = 0; k < FS_ADDR_COUNT; ++k)
					{
						if (thrid_addrs->addr_no[k] != 0)
						{
							...
							read_block(dev_id, thrid_addrs->addr_no[k], fs_b);
							if (p == NULL)
							{
								//申请内存给链表
